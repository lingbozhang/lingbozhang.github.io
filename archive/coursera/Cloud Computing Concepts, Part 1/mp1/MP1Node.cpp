...
/*
 *  Clean up node states and clear MemberListTable,
 */
void MP1Node::cleanNodeState() {
    memberNode->inGroup = false;
...
bool MP1Node::recvCallBack(void *env, char *data, int size ) {
    ...
    if(receivedMsg->msgType == JOINREQ) {
        ...
        /* add node to the MemberListTable */
        addNodeToMemberListTable(id, port, heartbeat, memberNode->timeOutCounter);
        
        /* get node address and send JOINREP message */
        ...
        unsigned long numberOfItems = memberNode->memberList.size();
        ...
        for(int i = 0; i < numberOfItems; i++) {
            ...
            /* create and add node to the MemberListTable */
...
void MP1Node::nodeLoopOps() {
    ...
    /* check if heartbeats should be sent */
    if(memberNode->pingCounter == 0)
    {
        ...
        for (auto& it : memberNode->memberList) {
            ...
            if(!(memcmp((char*)&(memberNode->addr.addr), (char*)&(nodeAddress.addr), sizeof(memberNode->addr.addr)) == 0)) {
                size_t msgsize = sizeof(MessageHdr) + sizeof(nodeAddress.addr) + sizeof(long) + 1;
                MessageHdr *msg = (MessageHdr *) malloc(msgsize * sizeof(char));
...
void MP1Node::addNodeToMemberListTable(int id, short port, long heartbeat, long timestamp) {
    ...
    /* create and insert  new MemberListEntry */
    if (!existsNodeInMemberListTable(id)) {
        auto newEntry = new MemberListEntry(id, port, heartbeat, timestamp);
